...
def ip_range():
    ...
    print (f'An array of all IPaddresses is assembled and represented in a variable "ips"\nips={ip_str}')
...
def connection_with_openstack():
    ...
    print (f'Succesfully connect with openstackSDK')  
    return conn    
...
def find_project_name(ip):
    ...
    print (f'List of servers was found with openstackSDK')
    ...
    if output_byIP_json == []:
        ...
        print (f'Target IP {ip} was not found in list of servers ')
        ...
        print (f'List of floating IPs was found because target IP was not in list of servers')
        ...
        if output_byfloatingIPs_json == []:
            ...
            print (f'Target IP {ip} was not found in list of floating IPs ')
        else:
            ...
            print (f'ProjectID in openstack of target ip: {ip} was found among floatingIPs and it is next : {found_projectID_openstack}')
    else:
    ...
    if projectID == "Ip Not Found":
        ...
        print (f'Project name not found because Ip {ip} Not Found not in servers_list not in list_of_flostingIPs')
        #logInsert(f'Function "find_project_name({ip})" completed, but project NAME not found in openstack')
        print (f'Function "find_project_name({ip})" completed, but project NAME not found in openstack')
    else:
        conn = connection_with_openstack()
        projects = conn.identity.projects()
        lst = list(projects)
        #logInsert(f'Project list in openstack was found')
        print (f'Project list in openstack was found')
        ...
        print (f'Project NAME was found and it is next : {found_projectNAME}')
        ...
        print (f'Project NAME formatted for Fleio: {projectNAME_Fleio}')
        ...
        print (f'Function "find_project_name({ip})" completed successfully - project NAME in openstack for Fleio is: {projectNAME_Fleio}')
...
def find_email_fleio(ip):
    ...
    print (f'Firstly, needs to define project NAME')
    ...
    if project_name == 'Project name not found':
        ...
        print (f'Email not found because Project name of target ip {ip} not defined even in openstack')
        #logInsert (f'Function "find_email_fleio({ip})" completed but email was not found')
        print (f'Function "find_email_fleio({ip})" completed but email was not found')
    else:
        found_email = False
        pages = []
        for i in range (1, 100):    
            myToken = fleio_token
            myUrl = str(f"{fleio_auth_url}/clients?page={i}")
            head = {'Authorization': 'token {}'.format(myToken)}
            #logInsert (f'Authorization in Fleio is succesfull')
            print (f'Authorization in Fleio is succesfull')
            ...
            if response_status == 200:
                ...
                print (f'Page {i-1} - response_status_code  = {response_status} so information about clients in FLEIO from page {i-1} was got')
            else:
                #logInsert (f'Pagging was ended, all information is collected in a variable "pages"')
                print (f'Pagging was ended, all information is collected in a variable "pages"')
                break
        #logInsert (f'Start browsing every page and analyzing each object on page')
        print (f'Start browsing every page and analyzing each object on page')
        for page in pages:
            for objects in page:
                if objects['name'] == project_name:
                    found_email =  (objects['email'])
                    #logInsert (f'Found email for project {project_name} is {found_email}')
                    print (f'Found email for project {project_name} is {found_email}')
                    break 
        #logInsert(f'Function "find_email_fleio({ip})" completed successfully - found email is {found_email}')   
        print (f'Function "find_email_fleio({ip})" completed successfully - found email is {found_email}')             
    return found_email     
...
def sendmail_report(to_adr, code):
    ...
    print (f'Start to form the body of the report, which will contain the scan results')
    html = """\
    <html>
      <head></head>
      <body>
    ...
    print (f'Log in to the mail from which reports will be sent (now it is {from_adr})')
...
def scanning():
    ...
    print (f'Start scanning each IPaddress according to the list of dangerous ports')
    ...
    print (f'Scanning is completed, all information is collected in a variable "nm_scan_str"')
    #logInsert(f'Start analazing which IPaddress has open dangerous ports')
    print (f'Start analazing which IPaddress has open dangerous ports')
    for host in nm.all_hosts():
        ...
        if open_ports != []:
            ...
            print (f'Analyzing is comleted: the host {host} has the following open dangerous ports: {output}')
        
            #logInsert(f'Start looking for contact details of {host} for sending a report')
            print (f'Start looking for contact details of {host} for sending a report')
            ...
            #logInsert(f'Analyzing is comleted: the host {host} has NO OPEN dangerous ports. ')
            print (f'Analyzing is comleted: the host {host} has NO OPEN dangerous ports. ')
            #logInsert(f'______________________________________________________________________________')
            print (f'______________________________________________________________________________')
